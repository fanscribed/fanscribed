...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for task in orm['transcripts.TranscribeTask'].objects.filter(state='valid').order_by('validated_at'):
            fragment = task.fragment
            if fragment is not None and fragment.last_editor is None:
                fragment.last_editor = task.assignee
                fragment.save()

        for task in orm['transcripts.StitchTask'].objects.filter(state='valid').order_by('validated_at'):
            stitch = task.stitch
            if stitch.last_editor is None:
                stitch.last_editor = task.assignee
                stitch.save()
                
        for task in orm['transcripts.BoundaryTask'].objects.filter(state='valid').order_by('validated_at'):
            sentence = task.sentence
            if sentence.boundary_last_editor is None:
                sentence.boundary_last_editor = task.assignee
                sentence.save()

        for task in orm['transcripts.CleanTask'].objects.filter(state='valid').order_by('validated_at'):
            sentence = task.sentence
            if sentence.clean_last_editor is None:
                sentence.clean_last_editor = task.assignee
                sentence.save()

        for task in orm['transcripts.SpeakerTask'].objects.filter(state='valid').order_by('validated_at'):
            sentence = task.sentence
            if sentence.speaker_last_editor is None:
                sentence.speaker_last_editor = task.assignee
                sentence.save()
